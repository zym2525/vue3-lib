<template>
  <div ref="containerDom" class="tags-view" v-if="!showTags">
    ...
    <transition name="el-zoom-in-top">
      <ul v-show="visible" :key="Math.random()" :style="getContextMenuStyle" class="contextmenu">
        <div v-for="(item, key) in tagsViews" :key="key" style="display: flex; align-items: center">
          <li v-if="item.show" @click="selectTag(key, item)" :key="key">
    ...
    <ul class="right-button">
      <li>
        <el-icon :title="'刷新'" class="el-icon-refresh-right rotate" @click="onFresh">
...
import close from '@/assets/svgIcons/close.svg?component'
import refresh from '@/assets/svgIcons/refresh.svg?component'
import closeAll from '@/assets/svgIcons/close_all.svg?component'
import closeLeft from '@/assets/svgIcons/close_left.svg?component'
import closeOther from '@/assets/svgIcons/close_other.svg?component'
import closeRight from '@/assets/svgIcons/close_right.svg?component'
...
const linkIsActive = computed(() => {
  return (item) => {
    if (route.path === item.path && isEqual(route?.query, item?.query)) {
      return 'is-active'
    } else {
      return ''
    }
...
function dynamicRouteTag(value: string, parentPath: string, query?: object): void {
  const hasValue = multiTags.value.some((item) => {
    return item.path === value && isEqual(item?.query, query)
  })

  function concatPath(arr: [], value: string, parentPath: string) {
    if (!hasValue) {
...
function handleCommand(command: { key: any; item: any }) {
...
function showMenuModel(currentPath: string, query: object = {}, refresh = false) {
  ...
  if (isEmpty(query)) {
    ...
    currentIndex = allRoute.findIndex((v) => isEqual(v.query, query) && v.path === currentPath)
  }
  ...
  } else if (currentIndex === 0 || currentPath === '/home') {
    // 当前路由为首页
    disabledMenus(true)
  } else {
    disabledMenus(false)
...
function openMenu(tag, e) {
  ...
  boleSetting.hiddenSideBar ? (buttonTop.value = e.clientY) : (buttonTop.value = e.clientY - 40)
  setTimeout(() => {
    visible.value = true
  }, 10)
}
...
onBeforeMount(() => {
  ...
  //  接收侧边栏切换传递过来的参数
  emitter.on('changLayoutRoute', ({ indexPath, parentPath, query }) => {
    dynamicRouteTag(indexPath, parentPath, query)
    setTimeout(() => {
